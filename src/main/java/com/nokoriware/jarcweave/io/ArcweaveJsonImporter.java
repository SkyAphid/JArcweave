...
import com.nokoriware.jarcweave.project.processed.AttributeValue;
import com.nokoriware.jarcweave.project.processed.Connection;
import com.nokoriware.jarcweave.project.processed.Cover;
...
public class ArcweaveJsonImporter {
	...
	private static void readGeneralInformation(JsonObject projectObject, RawArcweaveProject project) {
		...
		if (containsValidKey(projectObject, "startingElement")) {
	...
	private static void readCover(JsonObject projectObject, RawArcweaveProject project) {
		if (containsValidKey(projectObject, "cover")) {
	...
	private static void readBoards(JsonObject projectObject, RawArcweaveProject project) {
		...
		boards.forEach((boardID, boardValue) -> {
			...
			//Board notes
			if (containsValidKey(boardObject, "notes")) {
			...
			//Board jumpers
			if (containsValidKey(boardObject, "jumpers")) {
			...
			//Board branches
			if (containsValidKey(boardObject, "branches")) {
			...
			//Board elements
			if (containsValidKey(boardObject, "elements")) {
			...
			//Board connections
			if (containsValidKey(boardObject, "connections")) {
	...
	private static void readElements(JsonObject projectObject, RawArcweaveProject project) {
		...
		elements.forEach((elementID, elementValue) -> {
			...
			// title & content
			if (containsValidKey(elementObject, "title")) {
			...
			if (containsValidKey(elementObject, "attributes")) {
				JsonArray attributeArray = elementObject.getJsonArray("attributes");
				
				for (int i = 0; i < attributeArray.size(); i++) {
					String attributeID = attributeArray.getJsonString(i).getString();
					element.getAttributeIDs().add(attributeID);
				}
			}

			// attached component IDs
			if (containsValidKey(elementObject, "components")) {
			...
			//linked board
			if (elementObject.containsKey("linkedBoard") && elementObject.get("linkedBoard").getValueType() != ValueType.NULL) {
	...
	private static void readConnections(JsonObject projectObject, RawArcweaveProject project) {
		...
		connections.forEach((connectionID, connectionValue) -> {
			...
			// label
			if (containsValidKey(connectionObject, "label")) {
	...
	private static void readAttributes(JsonObject projectObject, RawArcweaveProject project) {
		...
		attributes.forEach((attributeID, attributeValue) -> {
			...
			String cID = attributeObject.getString("cId");
			String cType = attributeObject.getString("cType");
			
			RawAttribute attribute = new RawAttribute(attributeID, cID, cType);
			
			//Attribute name
			if (containsValidKey(attributeObject, "name")) {
